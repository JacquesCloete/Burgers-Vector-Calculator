...
% Calculate the directions of the reference Burgers vectors for
% the example material specimen:
b_ref = Reference_Burgers_Vectors_2D;

it_tot = 10;
magnitude_percentage_error = zeros(it_tot,1);
angular_separation = zeros(it_tot,1);
square_pixel_sidelength = zeros(it_tot,1);

for it = 1:it_tot
    % Define the x-limits of the input data in lab coordinates (in m)
    xlims = [(20+it-1)*3.9e-9 (40-it+1)*3.9e-9];    % x-limits (m)
    
    % Define the y-limits of the input data in lab coordinates (in m)
    % (Note the upper limit is the first entry for the y-direction)
    ylims = [(33-it+1)*3.9e-9 (13+it-1)*3.9e-9];    % y-limits (m)
    ...
    angular_separation(it) = real(acosd(dot(b_ref/norm(b_ref),b_computed/norm(b_computed))));    
    % Find the magnitude of the Burgers vector:
    b_magnitude = norm(b_computed);
    magnitude_percentage_error(it) = abs((3.1652e-10-b_magnitude*2/3^(1/2))/3.1652e-10*100);
    
    square_pixel_sidelength(it) = 21-(it-1)*2;
end

h1 = subplot(1,2,1);
plot(square_pixel_sidelength,magnitude_percentage_error,'b')
...
h2 = subplot(1,2,2);
plot(square_pixel_sidelength,angular_separation,'r')
ylim([0 25])
ylabel(['Angular Separation (' char(176) ')'])
grid on

[ax,h2]=suplabel('Burgers Circuit Side Length (No. Pixels)','x');

sgtitle('Variation of Computed Burgers Vector with Burgers Circuit Size - 2D Data')