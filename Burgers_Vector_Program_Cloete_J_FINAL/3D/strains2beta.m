function beta = strains2beta(strains)
% strains2beta
% Converts the strains array into the displacement gradient field, which we
% shall use to perform the numerical integration

% Create an empty 5D array in which to store the displacement gradient:
beta = zeros(size(strains));

% We must first extract the 3D lattice rotation and elastic strain fields
% from the 5D array:
