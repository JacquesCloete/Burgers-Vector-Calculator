...
cube_voxel_sidelength = zeros(21,1);
b_true = [1;1;1]*1e-10;
...
for it = 1:21
    ...
    cube_voxel_sidelength(it) = 42-(it-1)*2;
end

h1 = subplot(1,2,1);
semilogy(cube_voxel_sidelength,magnitude_percentage_error,'b')
ylabel('Error in Magnitude (%)')
grid on


h2 = subplot(1,2,2);
plot(cube_voxel_sidelength,angular_separation,'r')
ylabel(['Angular Separation (' char(176) ')'])
grid on

[ax,h2]=suplabel('Burgers Circuit Side Length (No. Voxels)','x');

sgtitle('Test 1 - Variation of Computed Burgers Vector with Burgers Circuit Size')